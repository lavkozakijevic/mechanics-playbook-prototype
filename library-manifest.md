...
> ✅ **Scope note resolved (16 Sep 2026):** the fabrication flag below refers to the original `sources/analyses/gymverse.md` (created 1 Jul 2026, 172 lines, in-file dated 25 Feb–20 May 2026), not the file that exists at that path today. That version carried two concrete fabrication markers: vocabulary with no place in a gym app (a "Bendometer" equivalent, "class stretch") sitting inside its Streak section, and a direct internal contradiction — its own "Category context" section stated "No explicit comparisons to other fitness apps were made in the transcript," while its "Unrecognized mechanics" section cited before-and-after photo tracking as "also mentioned in LazyFit and BetterMe," two apps with no place in this walkthrough.
>
> That file was fully replaced on 14 Sep 2026 by an independent 712-line re-analysis of the same underlying recording (not a patch of the flagged version), which is what `sources/analyses/gymverse.md` holds today and what the current Gymverse write-up and SYSTEMS entry are built from. Read in full on 16 Sep 2026 checking specifically for both patterns: no out-of-place vocabulary or cross-app citation appears anywhere, and every claim traces to a numbered observation with a stated confidence tier — including places where the old file smoothed over an inconsistency the new one flags directly instead (O40 records two different yearly prices seen in the transcript, $112.20 and $120, as an explicit unresolved sequence caveat rather than picking one and asserting it as fact). The current analysis is clean.
>
> Also stale below, unrelated to the fabrication question: this note and the tables that follow describe Gymverse as `subscriber` with a v3-style write-up. As of 16 Sep 2026 Gymverse is `report-only` for an unrelated reason, the three-publishable-tag minimum (owner ruling, 16 Sep 2026) — its v4.1 write-up is complete, at two tags, below the new floor. This manifest is a generated snapshot rather than a live document, so the rest of it is left as originally generated.
