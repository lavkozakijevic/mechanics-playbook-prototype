# Cleo: behavioral design analysis

**Session date:** 04 Jun 2026

**Additional sessions:** None.

**As observed:** Feb 2024
...
Source: one recorded walkthrough session, 04 Jun 2026, retrieved in full as a verbatim transcript. No other session for this app exists in the record. The narrator states that the walkthrough shows the app as it was in February 2024, and at one point states that the flow is being viewed through a source the transcript renders as "mobin" and that the flow is becoming broken; where order matters, this is marked with a sequence caveat. Interface copy is quoted as the narrator reads it aloud, so exact screen wording can differ, and garbled transcript renderings are flagged where they occur. Account amounts are given only where the narrator reads them. Profane copy in the app's messages is described, not quoted. The account's US state restricts the paid plans, and no paid feature was used.

## Evidence key

- directly observed: shown on screen or read aloud in the session.
- strongly supported: several observations converge and no strong alternative is present.
- plausible: fits the evidence while competing explanations remain.
- unresolved: the session does not settle it.

Order-of-flow limitations are marked separately as (sequence caveat: note).
...
**O1. Pre-account carousel leads with four money outcomes.**

**Observed:** The first screen is a carousel of four slides the user can swipe through, with a sign up button and an I already have an account button at the bottom (tier: directly observed). Each slide states one product outcome, ordered cash advance, spending limits, credit score, then savings (tier: directly observed).

**Detail:**
- Slide one reads "up to $500 in cash advances when you need a little extra" (tier: directly observed).
- Slide two reads "learn to spend better with spending limits and budget tips" (tier: directly observed).
- Slide three reads "stress less about your score" with "apply for the credit builder card" (tier: directly observed).
...
**Observed:** Tapping sign up opens a sequence of single-purpose screens: name, email, date of birth, US state, where the user heard about Cleo, and email verification by code (tier: directly observed). After the code is entered, the next screen reads "that totally worked" (tier: directly observed).

**Detail:**
- The name screen reads "add your full legal details" with first name and last name fields (tier: directly observed).
- The email screen reads "enter the email where you'd like to receive the code" (tier: directly observed).
- The state screen lists US states only (tier: directly observed).
- The attribution screen offers App Store, Play Store, Facebook, friend, family and further options (tier: directly observed).
...
**Observed:** Copy throughout the session is written in the voice of Cleo, referred to as "she", using jokes, slang and emojis, with large type, bright colors and illustrations on full screens (tier: directly observed). Chat messages are paired with GIFs chosen for the moment, including one matched to the amount the user has spent (tier: directly observed). Some messages contain profanity (tier: directly observed).

**Detail:**
- The bank connection screen says Cleo "can't move your money. She's just an AI with bad attitude", says she never stores bank login details, and adds "bit creepy if she did" (tier: directly observed).
- Slang forms include "qs" for questions, "v" for very, "peeps" for people, and a capital K for okay (tier: directly observed).
...
**Observed:** A "link your account" screen states that Plaid connects Cleo securely and that "you're in good company with millions of users", followed by the persona lines about not moving money or storing logins (tier: directly observed). Tapping "connect me" opens a Plaid pop-up, after which the user enters a phone number, chooses an institution, enters bank credentials and verifies identity with a code sent by text or email (tier: directly observed).

**Detail:**
- The screen offers "connect me" with a lightning emoji and a second option for more information about security (tier: directly observed).
- The Plaid pop-up lists "connect effortlessly", "your data belongs to you" with the statement that Plaid doesn't sell personal info and uses it only with permission, and "protect your accounts" describing fraud reduction using account info, transaction history and connection history (tier: directly observed).
- The narrator does not describe an option to skip the connection (tier: directly observed).
- The connected account is the source of the balances, transactions, bills and income suggestions shown later in the session (tier: strongly supported).

*Cross-reference: O11, O13, O19.*

**O5. An "are you ready?" screen launches the chat.**

**Observed:** After identity verification, a full screen reads "are you ready?" with large type, illustrations and a single button, "Unleash Cleo", with a sideways-eyes emoji (tier: directly observed). The button opens the chat (tier: directly observed).

**Detail:** None.

*Cross-reference: O6.*

**O6. The first chat opens with a greeting and a choice of starting goal.**

**Observed:** The chat opens with a typing indicator, and Cleo sends the first messages without any user input (tier: directly observed). Cleo greets the user by name with "I've been expecting you" and "you just walked into the best decision of your fabulous life", shows a GIF of a woman clapping captioned "knew you could do it", and asks "so where do we start?" (tier: directly observed).

**Detail:**
- The starting options, each with an emoji, are rendered as control my spending, get a cash advance, improve my credit score, and "I don't know, see where my money goes" (tier: directly observed).
...
**Observed:** After control my spending is chosen, Cleo says it will first ask about money habits, asks the user to be honest across "the next five qs", and states "the more I know the better I can help you" (tier: directly observed). Questions are answered by quick reply, and Cleo responds to each answer before the next question (tier: directly observed). The quiz ends with "now I've got enough ammo to set your worst habits on fire" and a button, "roast me Cleo" (tier: directly observed).

**Detail:**
- The first question asks how much of their income the user usually spent over the last six months, counting paychecks, government support and student loans, with options not much, most of it, about the same, all of it and then some, and all of it and then a lot more (tier: directly observed).
- The session selects all of it and then a lot more (tier: directly observed).
- A later question asks "what are you like with bills?" with options including "I'm on top of them" and "they're on top of me" (tier: directly observed).
- Cleo answers one reply with "okay, not here to judge you" (tier: directly observed).
- The remaining questions are not described individually (tier: directly observed).

*Cross-reference: O8.*

**O8. The roast turns the user's own account figures into a joke sequence.**

**Observed:** The roast is a chat sequence in which Cleo states figures from the user's account one at a time, each paired with a joke and a GIF, and the user advances it with quick replies (tier: directly observed). It closes with the total spent over the last 13 months shown as a large number with a GIF, then "okay, you're free now" (tier: directly observed). The messages restate the user's figures and quiz answers rather than critiquing individual purchases (tier: strongly supported, based on the narrator's description of the content).

**Detail:**
- The opening message uses the user's name with "let us descend into your madness" and a cat GIF, answered "yes, let's" (tier: directly observed).
- One message states the amount spent this month as the amount the user "sunk", with a GIF matched to that amount (tier: directly observed).
- Another states the amount received in the last two weeks with "I'm excited to see how creatively you waste it" (tier: directly observed).
- Quick replies include "keep them coming" and "I can take more" (tier: directly observed).
...
**Observed:** After the roast, Cleo says "you made it, your overview is ready", describes the overview as covering where money goes each month or pay cycle, and explains how it works: Cleo lays out what comes in and goes out, gives the user a daily spending limit, and the user checks their phone to stay on track (tier: directly observed). The user replies "I can do this", Cleo answers with a profane line, and a "my overview" button leads to "you're all set" (tier: directly observed). Cleo then says a spend limit keeps the overdraft away, "think of it as a harness there to protect you from financial chaos", with a "see updated budget" button (tier: directly observed).

**Detail:**
- The explanation arrives as message bubbles appearing one under another (tier: directly observed).
...
**Observed:** A screen reads "track your bills: bills suck but I can make them suck less. Confirm your bills here so I can track them for you", with a verify bills button (tier: directly observed). The pop-up asks "are these your bills?", defines bills as recurring or fixed payments, and lists payments drawn from the connected account (tier: directly observed). The next screen reads "like a boss" with "nice job, you fully customized your budget" and a "take a look" button that opens the budget (tier: directly observed).

**Detail:**
- The pop-up instructs the user to tap to edit or swipe to remove (tier: directly observed).
- The congratulation screen uses background illustrations (tier: directly observed).
- No step in which the user sets a limit amount is described before the "fully customized" message (tier: directly observed).

*Cross-reference: O9, O12, O13.*

## Core loop and automation

**O11. The spend tab shows linked accounts and recent transactions.**

**Observed:** The spend tab opens with swipeable account cards at the top, the first showing the connected checking account and its balance, and a second card reading "connect to bank" (tier: directly observed). Below are add account and manage account buttons and a recent transactions list with view more, which shows transactions from this month and earlier (tier: directly observed).

**Detail:**
- Add account reopens the Plaid connection flow (tier: directly observed).
- Manage account lists active accounts with toggles that make each one visible or invisible, and a connect another account button (tier: directly observed).
- An edit button on recent transactions opens a pop-up, "select how many transactions you want to see on the spend tab", with up and down controls (tier: directly observed).

*Cross-reference: O4.*

**O12. The budget tab summarizes the month with coin stacks and a per-day figure.**

**Observed:** The budget tab shows this month's breakdown with the date and the number of days left in the month, two stacks of coins for money in and money out, and figures for the limit, the amount spent and the amount due (tier: directly observed). Below are a larger figure followed by "until" and a second figure, and a per-day amount (tier: directly observed). The chat card in O15 renders the same pair as 1,200 left of a 1,400 budget, so the figures read as the amount remaining and the monthly limit (tier: strongly supported).

**Detail:**
- The narrator reads the per-day figure as how much the user can spend per day, without reading its on-screen label (tier: plausible).
- A widget rendered as "been two weeks of budget thoughts" invites feedback on budgeting so far (tier: directly observed).
...
**Observed:** Scrolling the budget tab shows bills due, listing items such as subscriptions, with an option to add more bills (tier: directly observed). An arrow opens a pop-up reading "are any of these bills? tap a transaction to add it as a bill", listing earlier transactions as suggestions (tier: directly observed). Tapping one shows its amount, frequency and next payment date, and adds it to bills (tier: directly observed).

**Detail:**
- The user can view all bills and change their details (tier: directly observed).
- The suggestions come from transactions on previous days and earlier (tier: directly observed).

*Cross-reference: O10, O20.*

**O14. Spending is split into essential and other categories that the user can re-sort.**

**Observed:** The spending section shows two swipeable cards, essential spending and other spending, each with the amount spent and its percentage of income, and swiping between them changes the category breakdown shown below (tier: directly observed). An edit categories button opens "sort your spends: tap to add or remove your categories", where categories can be moved between essential and other and saved (tier: directly observed).

**Detail:**
- Other spending shows zero in this session (tier: directly observed).
- The category breakdown lists categories such as transport and transfer out, with budget limits where set (tier: directly observed).
...
**Observed:** The chat tab opens on cards rather than a message thread: today's money insights, explore, get smarter, a game card, stay on top, where's it going, and need help, with a text box for typing to Cleo (tier: directly observed). Each card carries buttons that start a guided conversation or open a view (tier: directly observed).

**Detail:**
- Today's money insights holds "check with Cleo: can you afford that thing?" and "upcoming bills: coming out soon", with the amount and the account it comes from (tier: directly observed).
- Explore holds "need a boost? it's hype mode" (tier: directly observed).
- Get smarter holds "a five minute read all about gambling addiction: what to look for and how to get help" (tier: directly observed).
- The game card reads "oh, it's game time. I think you know your money? Play the game with self respect", with play, game stats and opt out buttons (tier: directly observed).
- Stay on top reads "you're good, 1,200 left of your 1,400 budget" with an end date and buttons for income, bills and budget; income opens the income received this month (tier: directly observed).
- Where's it going reads "good to check every day" with buttons for balance, top spends, whole month and make Cleo smarter (tier: directly observed).
...
**Observed:** After each chat card has been used, it shows a green check mark (tier: directly observed). The narrator reads the mark as recording which cards have already been seen or completed (tier: directly observed).

**Detail:**
- The first group of cards is headed "today's" (tier: directly observed).
- No reward or unlock is described in connection with the marks (tier: directly observed).
- The session does not show whether the marks clear or the cards change on a later day (tier: directly observed).

*Cross-reference: O15.*

**O17. "Can you afford that thing?" checks a purchase against balance and daily spend.**

**Observed:** The affordability conversation asks the user to visualize the thing they want to buy, then asks roughly how much it costs (tier: directly observed). After an amount is entered, Cleo states the user's current spending per day and balance across all accounts, says the user is good for the amount, and offers a "set income" button (tier: directly observed).

**Detail:**
- The first quick reply is "visualizing" (tier: directly observed).
- The closing message includes "onwards and upwards" (tier: directly observed).

*Cross-reference: O15, O19.*

**O18. Typed questions return a monthly overview.**

**Observed:** Typing "how much did I save this month?" returns an overview from the start of the month to the current date showing income, spent and net (tier: directly observed). Three options follow: by categories, by merchants, and to my insights feed (tier: directly observed).

**Detail:**
- The narrator does not locate the insights feed and does not open it (tier: directly observed).

*Cross-reference: O15, O23.*

**O19. Income setup suggests detected deposits or accepts manual entry.**

**Observed:** Set income opens a screen asking the user to add their income so Cleo knows what is coming in, stating that she will guide the user on what they can let out (tier: directly observed). With no active income, it states that it found some incomes and offers add payroll deposits, add miscellaneous, or add income manually (tier: directly observed). Manual entry asks who is paying, how often, when the next payment is and how much the next paycheck is, then shows upcoming paydays before "all done" (tier: directly observed).

**Detail:**
- The detected incomes are deposits that have been coming into the connected account (tier: directly observed).

*Cross-reference: O4, O12, O17.*

**O20. The upcoming bills conversation ends in a keep-or-ditch swipe of subscriptions.**

**Observed:** Upcoming bills opens an animated view with "bills" in large letters, a large floating poo emoji and a GIF, with quick replies including "live laugh bills" (tier: directly observed). Cleo states the total bills due before payday, asks "what does your balance have to say about this?" with thumbs up or thumbs down, and after thumbs down asks whether all the bills are must-haves or whether there are places to cut back (tier: directly observed). Cleo then says some bills keep the lights on while others "mystically appear" when free trials end, and asks the user to swipe right on a bill to keep and left on "something you might want to ditch", presenting subscriptions as swipeable cards (tier: directly observed).

**Detail:**
- One quick reply is offered in two spellings, "maybe" and a joke spelling (tier: directly observed).
- What happens to a subscription after a left swipe is not described (tier: directly observed).

*Cross-reference: O3, O13, O26.*

**O21. Hype mode is an enthusiasm sequence built from account figures and ends in a review request.**

**Observed:** "Need a boost? It's hype mode" opens a chat in which, after the user taps "hype me" and "my spirit is ready", Cleo promises enthusiasm, "stats, memes, GIFs, all the emojis" (tier: directly observed). Cleo then states the amount spent this month, the bank charges this month with an offer to send the bank "some stern GIFs", the total money made, and the biggest recent purchase, each with quick replies (tier: directly observed). The sequence closes with "we're done here" and a request to leave a review, answered "bring it" (tier: directly observed).
...
**Observed:** Opening the gambling addiction card starts a chat in which Cleo calls it "not a super fun topic but a v real one", cites a figure of 46 million "peeps" in the US, notes that much of it happens online, and says there are ways to help, "many of them here in this blog, five minute read" (tier: directly observed). The options are to go to the blog or back to insights, which returns to the chat home (tier: directly observed).

**Detail:**
- The article itself is not opened in the session (tier: directly observed).

*Cross-reference: O15.*

**O23. Balance, graph and dashboard views sit behind the where's it going card.**

**Observed:** Balance shows the balance for the connected bank and overall in the chat, with when it was last updated, and offers graph, dashboard, home and refresh accounts (tier: directly observed). The graph shows the balance trend across the past three months, and the dashboard shows this month's transactions, categories and merchants in three tabs (tier: directly observed).

**Detail:**
- Refresh accounts refreshes the account information (tier: directly observed).

*Cross-reference: O15, O18.*

**O24. "Make Cleo smarter" asks the user to categorize transactions in the chat.**

**Observed:** Make Cleo smarter opens a chat: "great, I'm going to show you a few transactions, tell me which category they belong to, ready?" (tier: directly observed). After "I'm ready", Cleo shows an unsorted transaction with a "choose category" button that lists six or seven suggested categories and a more categories option (tier: directly observed).

**Detail:**
- Categorization is done one transaction at a time through the conversation rather than in a settings list (tier: directly observed).
- What changes after a category is chosen is not described (tier: directly observed).

*Cross-reference: O14, O15.*

**O25. The money game quizzes the user on their own recent account activity.**

**Observed:** Play opens a chat quiz whose questions concern the user's own account activity, such as whether any bills were paid this week and how much was spent in the last seven days (tier: directly observed). Each answer is judged against the account data: answering yes to the bill question when no bills are recorded returns "sadly not correct", and a wrong spending amount returns "nope" (tier: directly observed). After several questions, the last option is game stats (tier: directly observed).

**Detail:**
- The bill question is yes or no, and its feedback includes "you can add more here by the way" with a route to add bills (tier: directly observed).
- The spending question offers three amounts (tier: directly observed).
...
**Observed:** Start review first opens a practice round, "rate your spends: swipe right or hit loved if it was a purchase you don't regret", over a stack of cards with regret, loved and a don't-know emoji button, counted as one of three (tier: directly observed). The practice round ends with "that's all, you're set. The more you play the more trends you'll spot so I can help you hit your goals", and the real round then presents the user's payments one by one (tier: directly observed). Completion shows "you're done for today" and a loading sequence, then "nicely done, ratings are in", summarizing loved and regretted spends with their totals and listing the don't-know items (tier: directly observed).

**Detail:**
- The first review is labeled as covering 20 spends (tier: directly observed).
- The practice round has three cards (tier: directly observed).
...
**Detail:**
- The month view counts the days left until the month ends (tier: directly observed).
- The stay on top chat card states the amount left of the budget and when it ends (tier: directly observed).
...
**Detail:**
- The stats follow a single play in which the described answers were judged incorrect (tier: directly observed).
- The streak value is one while the knowledge score is 0% (tier: directly observed).
- The rule that increases or resets the streak is not shown (tier: directly observed).
...
**Detail:**
- One tip reads "you're regretting that McDonald's, huh" with the amount spent and a line about what else it could have bought (tier: directly observed).
- Another tip, on account fees, reads "you're into that, well, you do you" with the amount and "well spent, I guess" (tier: directly observed).
...
**Detail:**
- The narrator ties the change to completing onboarding (tier: directly observed).
...
**Detail:**
- Both requests arrive inside the chat, as the next message after the sequence's final figure (tier: directly observed).
- The only reply described for each request is affirmative (tier: directly observed).
- Where "I'll do it" and "bring it" lead is not described beyond leaving a review (tier: directly observed).
...
## Entries not observed

- **Clan / Guild.** No persistent collective entity or group-level state is shown in the session (tier: directly observed).
- **Daily Login Rewards.** No benefit is tied to accessing the app during a daily window (tier: directly observed).
- **Earning Tasks.** No catalogue of compensated tasks is shown (tier: directly observed).
- **Energy.** No depletable capacity constrains any action (tier: directly observed).
- **First-Purchase Bonus.** No purchase is made and no first-purchase benefit is described (tier: directly observed).
- **Gifting.** No route for directing a benefit to another person is shown (tier: directly observed).
- **Group Membership.** No system-recognized group is shown (tier: directly observed).
- **Hard Currency.** No product-defined units are offered for sale (tier: directly observed).
- **Leaderboard.** No ordered list of multiple users is shown (tier: directly observed).
...
- **Variable Reward Schedule.** No reward events occur in the session (tier: directly observed).

---

# Pass three: proposed new tags

### `assistant training prompt`

**Draft definition.** The product asks the user to label their own uncategorized records through a guided conversation framed as making the assistant smarter, one record at a time with suggested labels.

**Source observations:** O24.

**Conditions it appears to depend on:** Uncategorized records in the user's data (tier: directly observed). A conversational interface (tier: directly observed).

**Why it is not covered.** Personal Data Reflection covers interactions in which the product hands the user's records back to the user, and excludes interactions in which the user labels, categorizes, confirms or corrects records so that the product's classification improves, because the output flows to the product rather than to the user. O24 asks the user to choose a category for each transaction under the framing "make Cleo smarter", which is that second case (tier: directly observed). Earning Tasks does not apply because no compensation is disclosed (tier: directly observed).

**Recurrence elsewhere:** Not established from the source material, which is limited to this session.

**Caveat.** Only the start of one sequence is described, and what the labels change is not observed (tier: directly observed).
