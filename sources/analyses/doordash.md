...
**Tag:** Reviews and Ratings

**Observations:** O33, O32.

**Supporting observations:** O25, O18, O11.

**Confidence:** strongly supported.

**Role:** social, engagement.

DoorDash attributes contributions to the account and states that public contributions are visible to anyone, including people ordering in the reviewer's neighborhood, and display on the store page (O32, O33), and it points the user toward sharing reviews and photos as something to do (O25, O33).

**Rationale:** Exclusion conditions checked first. The ratings, rating counts, rating filter and review excerpts shown to a user choosing where to order are commerce infrastructure and carry no label, so the tag does not rest on the merchant card ratings, the deals screen filter or the customer favorites list (O8, O10, O11) (tier: directly observed). The badge ladder that counts contributions is classified separately as Achievement (O25) (tier: directly observed). The contributor page is held as the open Contributor Profile proposal in Pass Three and is not absorbed into this tag (O33) (tier: directly observed). DoorDash invites the user to contribute, through the contributor explainer's "Share your thoughts. Become a trusted voice by sharing reviews and earning badges" and through a badge criterion that counts stores for which high quality reviews or photos are shared (O25, O33) (tier: directly observed). It holds a state for contributions addressed to the contributor: "0 contributions. All contributions will appear once they're approved", a restricted profile setting that limits what others see while stating that public contributions still display on the store page, and a count at zero (O25, O33) (tier: directly observed). Each store page opens a ratings and reviews view and a customer photos view of what other customers have posted (O18, O32) (tier: directly observed). Under the presence standard, the product's own invitation and these contributor-side states establish the mechanic (tier: strongly supported, no contribution control was opened, nothing was rated, reviewed or photographed, and no order was placed). The approval step, the visibility setting, the count and the trusted-voice framing are shared with the open Contributor Profile proposal; this tag reads them as DoorDash's handling of contributions, and the page they sit on stays with the proposal (tier: strongly supported, the division follows the Reviews and Ratings entry's exclusion for a page gathering one contributor's contributions). The target behavior the surface points toward is sharing reviews or photos of stores (tier: directly observed). Whether anything obstructs that behavior has no evidence in the session (tier: unresolved, no contribution was attempted and no user response is recorded). The stated visibility rule for contributions is consistent with the Social Visibility pattern (tier: plausible, the rule is stated in copy and no published contribution of the user's is seen). Relatedness Need is a candidate principle for the "Be a reliable voice for other customers" and "Gather support for local business" framing, and it is not supported by evidence of user response (tier: unresolved, the session contains no behavior around contributing).

**Variants present:** rating contribution (store pages count public ratings alongside reviews, O32); written contribution (public reviews, O32, O33); photo contribution (customer photos on store pages, O18, O32); interchangeable-format contribution (reviews or photos count toward one badge criterion, O25); moderated contribution (contributions appear once approved, O33); listing-published contribution (public contributions display on the store page, O33); aggregated contribution (the store's count of public reviews and ratings, and a customer favorites list based on customer ratings, O11, O32); excerpted contribution (comment excerpts on the customer favorites screen, O11); restrictable contributor identity (a restricted profile shows first name, last initial and the number of public contributions, O33); profile-listed contribution (the explainer states all of the user's contributions appear in one place, O33, with the page held under the open Contributor Profile proposal); counted contribution (0 of 3 toward Emerging expert, O25); distinct-listing count (the criterion counts stores rather than contributions, O25); quality-qualified count (only high quality reviews or photos count, and the standard is not stated, O25); framed invitation (visibility, being a reliable voice for other customers, and support for local business, O25, O33).

**Variants not established:** unmoderated contribution (every contribution is stated to await approval); rewarded contribution (the badges state visibility and ranking benefits that are not seen to operate, and no balance, credit or offer is tied to contributing); awardable contribution (no mark placed by other users on a contribution appears). The contribution control, the rating scale, whether contributing requires a completed order, whether a prompt follows an order, what the approval step checks and how long it takes are not observed.

**Alternative considered:** No label, treating the ratings, reviews and photos as a commerce display described in plain language (tier: plausible). That reading would hold if the invitation and contributor-side states were copy with no working loop behind them, since the session shows every one of them at zero (O25, O33) (tier: directly observed). What would separate the two is a session that submits a review or photo and follows it through approval to the store page and the contributor's count.
...
- Community Space. Considered for store ratings, reviews and customer photos, for the contributor profile and for the group order. The entry excludes reviews, ratings and photos attached to commerce listings, and Reviews and Ratings, applied above, classifies DoorDash's invitation to contribute them and its handling of what users contribute, while treating the ratings shown to a user choosing a store as commerce infrastructure; classifying the same surface as Community Space would contradict the entry it is classified against. The contributor profile is held as an open proposal in Pass Three, and the group order is excluded as a single transaction (O31, O32, O33).
